...
async function loadProjectIssues() {
    ...
    try {
        ...
        docs.forEach(i => {
            ...
            <td class="px-4 py-4">
                <button onclick="window.changeStatus('${i.id}','${i.status}')" class="inline-flex items-center gap-1 rounded-md ${statusBg} px-2 py-1 text-xs font-medium ${statusColor} ring-1 ring-inset ring-white/10 hover:ring-white/30 transition-all">
...
window.changeStatus = async (id, oldStatus) => {
    ...
    else if (oldStatus === "Done") newStatus = "Open";
