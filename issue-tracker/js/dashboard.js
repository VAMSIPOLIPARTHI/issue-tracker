...
window.renderIssues = () => {
    ...
    filtered.forEach(i => {
        ...
        row.innerHTML = `
            ...
            <td class="px-4 py-4">
                <button onclick="changeStatus('${i.id}','${i.status}')" class="inline-flex items-center gap-1 rounded-md ${statusBg} px-2 py-1 text-xs font-medium ${statusColor} ring-1 ring-inset ring-white/10 hover:ring-white/30 transition-all">
                    <span class="material-symbols-outlined text-[14px]">${statusIcon}</span> ${i.status}
                </button>
            </td>
            <td class="px-4 py-4">
...
window.changeStatus = async (id, oldStatus) => {
    let newStatus = oldStatus;
    if (oldStatus === "Open") newStatus = "In Progress";
    else if (oldStatus === "In Progress") newStatus = "Done";
    else if (oldStatus === "Done") newStatus = "Open"; // Cyclic for demo or stop?

    if (oldStatus === "Open" && newStatus === "Done") {
        console.warn("Skipping steps not allowed");
        return;
    }
